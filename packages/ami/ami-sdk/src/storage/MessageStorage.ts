...
export class MessageStorage implements IMessageStorage {
    ...
    public async add(message: Message): Promise<boolean>{
        ...
        let messageThreadStorage = new MessageThreadStorage(this.messagesStorage , this.threadStorage , thid);
        
        return await messageThreadStorage.add(message);
    }

    public async get(id: string, thid?: string): Promise<Message>{
        if(thid){
        ...
        return this.messagesStorage.get<Message>(id) 
        
    }


    public async getByThread(thid: string): Promise<IMessageThreadStorage>{
        const messageThreadStorage = new MessageThreadStorage(this.messagesStorage , this.threadStorage , thid);

        return messageThreadStorage;
    }
    public async remove(message: Message) : Promise<boolean>{
        const messageThreadStorage = new MessageThreadStorage(this.messagesStorage , this.threadStorage , message.thid);
        return await messageThreadStorage.remove(message.id)
    }

    public async removeById(id: string, thid?: string): Promise<boolean> {
        const message = await this.messagesStorage.get<Message|undefined>(id);
        if(!message)
            return false
...
export class MessageThreadStorage implements IMessageThreadStorage {
    ...
    public async getAll(): Promise<Message[]> { 
        ...
        const messagePromises = ids.map( x => this.messagesStorage.get<Message>(x))
        return (await Promise.all(messagePromises)).filter(x => x) || [];
    }

    public async getByIndex(index: number): Promise<Message> {
        if(index < 0 )
            return;
        const ids = await this.getThreadMessagesId();
        if(index >= ids.length )
        ...
        return await this.messagesStorage.get<Message>(ids[index]);
        
    }
    public async get(id: string): Promise<Message> { 
        return await this.messagesStorage.get<Message>(id);
    }
